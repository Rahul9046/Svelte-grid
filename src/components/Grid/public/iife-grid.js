...
var Grid = (function () {
    ...
    function create_fragment(ctx) {
    	...
    	let div2;
    	let current;
    	var switch_value = ctx.features.search && ctx.features.search.getApp();
    	...
    	if (switch_value) {
    		...
    		switch_instance0.$on("searchApplied", ctx.handleUpdate);
    	}

    	var switch_value_1 = ctx.features.GlobalSort && ctx.features.GlobalSort.getApp();
    	...
    	var switch_value_2 = ctx.features["row-jumpers"] && ctx.features["row-jumpers"].getApp();

    	function switch_props_2(ctx) {
    		return {
    			props: { startRow: "20", endRow: "50" },
    			$$inline: true
    		};
    	}

    	if (switch_value_2) {
    		var switch_instance2 = new switch_value_2(switch_props_2());
    		switch_instance2.$on("rowSelectionChanged", ctx.handleRowUpdate);
    	}
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(div0, file, 18, 0, 398);
    			attr_dev(div1, "class", "sort-container");
    			add_location(div1, file, 21, 0, 542);
    			attr_dev(div2, "class", "row-jumpers");
    			add_location(div2, file, 25, 0, 680);
    		},
    		...
    		m: function mount(target, anchor) {
    			...
    			if (switch_instance2) {
    				mount_component(switch_instance2, div2, null);
    			}

    			current = true;
    		},
    		p: function update(changed, ctx) {
    			if (switch_value !== (switch_value = ctx.features.search && ctx.features.search.getApp())) {
    				if (switch_instance0) {
    					group_outros();
    			...
    			if (switch_value_1 !== (switch_value_1 = ctx.features.GlobalSort && ctx.features.GlobalSort.getApp())) {
    				if (switch_instance1) {
    					group_outros();
    			...
    			if (switch_value_2 !== (switch_value_2 = ctx.features["row-jumpers"] && ctx.features["row-jumpers"].getApp())) {
    				if (switch_instance2) {
    					group_outros();
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	const writable_props = ["features", "events", "datatable"];
    	...
    	$$self.$set = $$props => {
    		if ("features" in $$props) $$invalidate("features", features = $$props.features);
    		if ("events" in $$props) $$invalidate("events", events = $$props.events);
    		if ("datatable" in $$props) $$invalidate("datatable", datatable = $$props.datatable);
    	};

    	$$self.$capture_state = () => {
    		return { features, events, datatable };
    	};
    ...
    class App extends SvelteComponentDev {
    	constructor(options) {
    		super(options);
    		init(this, options, instance, create_fragment, safe_not_equal, { features: 0, events: 0, datatable: 0 });
    		...
    		if (ctx.features === undefined && !("features" in props)) {
    			console.warn("<App> was created without expected prop 'features'");
    		}

    		if (ctx.events === undefined && !("events" in props)) {
    			console.warn("<App> was created without expected prop 'events'");
    		}

    		if (ctx.datatable === undefined && !("datatable" in props)) {
    			console.warn("<App> was created without expected prop 'datatable'");
    		}
    	}
    ...
    class Grid{
    	constructor(config){
    		...
    		this._evt = {};
    		this.featureConfigs = {};
    		this.dataTable = getDataTable().getData();
...
//# sourceMappingURL=iife-grid.js.map
